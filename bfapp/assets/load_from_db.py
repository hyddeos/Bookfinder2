from bfapp.models import Book, Genre, User, UserBook, UserList, Publisher
...
def load_filterd_books(user, page_number, search_terms, list_type):
    ...
    if user:
        if list_type == "undesided":
            ...
            queryset = Book.objects.filter(id__in=book_ids).prefetch_related(
                "publisher"
            )

        elif list_type == "maybe":
            user_lists = UserList.objects.filter(user=user)
            for user_list in user_lists:
        ...
        # Advance/user choosen filtering
        q_filter = Q()
